...
(function(){
  ...
  function paint(state,data){
    const humanCount=data && data.humans!=null ? data.humans : null;
    const botCount=data && data.bots!=null ? data.bots : null;
    const split=(humanCount!=null && botCount!=null) ? `${humanCount}H + ${botCount}B` : null;
    const population=split || (data && data.players!=null ? String(data.players) : null);
    const capacity=population && data && data.maxPlayers!=null ? `${population} / ${data.maxPlayers}` : population;
    document.querySelectorAll('.public-server-card').forEach(el=>{
    ...
    indicators.forEach(el=>{
      el.classList.remove('online','offline','checking');
      el.classList.add(state);
      const suffix=capacity ? ` · ${capacity}` : '';
    ...
    labels.forEach(el=>{
      ...
      el.classList.add(state);
      if(state==='online'){
        const players=capacity ? ` · ${capacity}` : '';
        el.textContent='ONLINE'+players;
      } else if(state==='checking') el.textContent='CHECKING';
      else el.textContent='OFFLINE';
    });
    document.querySelectorAll('[data-live-state]').forEach(el=>el.textContent=state==='online'?'ONLINE':'OFFLINE');
    document.querySelectorAll('[data-live-map]').forEach(el=>el.textContent=data?.map||'Unavailable');
    document.querySelectorAll('[data-live-humans]').forEach(el=>el.textContent=humanCount==null?'—':humanCount);
    document.querySelectorAll('[data-live-bots]').forEach(el=>el.textContent=botCount==null?'—':botCount);
    document.querySelectorAll('[data-live-capacity]').forEach(el=>el.textContent=data?.maxPlayers==null?'—':data.maxPlayers);
    document.querySelectorAll('[data-live-telemetry]').forEach(el=>el.textContent=data?.telemetry?'ACTIVE':'UNAVAILABLE');
    document.querySelectorAll('[data-live-join]').forEach(el=>{
      const address=data?.joinAddress||'play.spawnmap.ai:7777';
      el.textContent=address;
      if(data?.join) el.setAttribute('href',data.join);
    });
  }
  ...
  document.querySelector('[data-copy-server]')?.addEventListener('click',async event=>{
    ...
    try{
      await navigator.clipboard.writeText('play.spawnmap.ai:7777');
      button.textContent='COPIED';
      window.setTimeout(()=>button.textContent='COPY ADDRESS',1300);
    }catch(_){
      button.textContent='play.spawnmap.ai:7777';
    }
  });
})();
