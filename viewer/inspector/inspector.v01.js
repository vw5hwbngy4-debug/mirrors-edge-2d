(function () {
  ...
  class SpawnMapInspector {
    ...
    _buildUI() {
      ...
      ui.className = 'spawnmap-inspector-ui';
      ui.innerHTML = `
        <div class="smi-top">
          <div class="smi-chip"><i class="smi-dot"></i><div><strong>SPAWNMAP INSPECTOR v0.1</strong><span> · PLAYER-HEIGHT DESIGN PREVIEW</span></div></div>
          <div class="smi-buttons">
            <button class="smi-button" data-smi="overview">OVERVIEW</button>
            <button class="smi-button primary" data-smi="inspector">INSPECT</button>
        ...
        <button class="smi-enter">ENTER INSPECTOR<span>WASD / TOUCH · WALK THE GENERATED MAP</span></button>
        <div class="smi-crosshair"></div>
        <div class="smi-readout"><b>INSPECTOR COORDINATES</b><span data-smi-readout>overview</span></div>
        <div class="smi-prompt">
          <input data-smi-note maxlength="320" placeholder="Change what you see here… e.g. make this alley narrower">
          <button data-smi-copy>COPY ITERATION PROMPT</button>
        </div>
        <div class="smi-help"><b>DESKTOP</b> WASD · mouse look · Shift sprint · Space jump · Esc releases mouse<div class="smi-runtime-note">Inspector validates design intent. UE1 skybox, BSP, pathing and gameplay remain runtime checks.</div></div>
        <div class="smi-mobile">
          <div class="smi-joystick"><div class="smi-stick"></div></div>
          <button class="smi-jump">JUMP</button>
          <div class="smi-look-hint">DRAG WORLD TO LOOK</div>
        </div>`;
    ...
    _bind() {
      ...
      const bindCanvas = () => {
        ...
        c.addEventListener('pointerdown', e => {
          if (this.mode !== 'inspector' || e.pointerType === 'mouse') return;
          this._touchLookId = e.pointerId;
          this._touchLookLast = { x:e.clientX, y:e.clientY };
          c.setPointerCapture && c.setPointerCapture(e.pointerId);
        });
        c.addEventListener('pointermove', e => {
          if (this.mode !== 'inspector' || e.pointerId !== this._touchLookId || !this._touchLookLast) return;
          const dx = e.clientX - this._touchLookLast.x;
          const dy = e.clientY - this._touchLookLast.y;
          this._touchLookLast = { x:e.clientX, y:e.clientY };
          this.yaw -= dx * 0.005;
          this.pitch -= dy * 0.005;
          this.pitch = Math.max(-1.42, Math.min(1.42, this.pitch));
        });
        const clearTouch = e => { if (e.pointerId === this._touchLookId) { this._touchLookId = null; this._touchLookLast = null; } };
        c.addEventListener('pointerup', clearTouch); c.addEventListener('pointercancel', clearTouch);
      };
    ...
    _setInnerUI(inspecting) {
      ...
      if (panel) panel.style.display = inspecting ? 'none' : '';
      if (toolbar) toolbar.style.display = inspecting ? 'none' : '';
      if (status) status.style.display = inspecting ? 'none' : '';
      if (tooltip) { tooltip.style.visibility = inspecting ? 'hidden' : ''; tooltip.style.display = 'none'; }
    ...
    enterOverview() {
      ...
      this.preview.fitCamera && this.preview.fitCamera('3d');
      this._updateReadout();
    }
    ...
    _updateReadout(){
      ...
      if(this.mode!=='inspector'){this.readout.textContent='Overview mode · choose INSPECT to walk at player height';return;}
